...
class AnalyzeStockPostUseCase:
    ...
    async def execute(self, raw_text: str) -> PostAnalysisResult:
        """
        ...
        """
        # OpenAI로 분석 (title, content, keywords 추출)
        analysis = await self.openai_service.analyze_stock_post(raw_text)
